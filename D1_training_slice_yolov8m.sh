...
slice_data() {
    ...
    python3 D1_training_with_slice.py \
        --data-root "${DATA_ROOT}" \
        --slice-dir "${SLICE_DIR}" \
        --crop-size ${CROP_SIZE} \
        --gap ${GAP} \
...
run_sahi_inference() {
    ...
    python3 D1_inference_with_sahi.py \
        --model "${model_path}" \
        --source "${source_dir}" \
        --save-dir "${save_dir}" \
...
# 定义模型列表
declare -a MODELS=("yolov8m.pt")
declare -a MODEL_NAMES=("yolov8m")
