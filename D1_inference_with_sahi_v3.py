...
"""
D1 数据集 SAHI 切片推理脚本 v3
使用训练好的模型对大尺寸图像进行切片推理
v3新增: 跨标签NMS，解决同一个孔洞被多个类别重复检测的问题
"""
...
def cross_class_nms(predictions, iou_threshold=0.5):
    """
    跨类别NMS：对所有类别的检测框进行NMS，去除重复检测
    用于解决SAHI推理时同一个目标被多个类别检测的问题
    ...
    # 返回保留的预测结果
    kept_predictions = [predictions[i] for i in keep]
    
    return kept_predictions


class BalloonSAHIInference:
    """D1 数据集 SAHI 切片推理类 v3 - 支持跨标签NMS"""
    ...
    def predict_image(
        ...
        Args:
            ...
            cross_class_nms_enabled (bool): 是否启用跨类别NMS
            cross_class_nms_threshold (float): 跨类别NMS的IoU阈值
        ...
        try:
            ...
            LOGGER.info(f"   跨类别NMS: {'启用' if cross_class_nms_enabled else '禁用'}")
            ...
            result = get_sliced_prediction(
                ...
                postprocess_class_agnostic=cross_class_nms_enabled,  # v3: 启用跨类别NMS
            )
        ...
        LOGGER.info(f"   面积过滤后: {len(filtered_predictions)} 个检测框")
        
        # v3新增: 额外的跨类别NMS（如果SAHI内置的不够，可以再做一次）
        if cross_class_nms_enabled and len(filtered_predictions) > 1:
            pre_nms_count = len(filtered_predictions)
            filtered_predictions = cross_class_nms(filtered_predictions, cross_class_nms_threshold)
            if len(filtered_predictions) < pre_nms_count:
                LOGGER.info(f"   跨类别NMS: {pre_nms_count} -> {len(filtered_predictions)}")
        ...
        if len(filtered_predictions) > max_detections:
            ...
            LOGGER.info(f"   检测框数量限制: {len(filtered_predictions)} -> {max_detections}")
        ...
        LOGGER.info(f"   最终检测到 {num_detections} 个目标")
    ...
    def predict_directory(
        ...
        LOGGER.info(f"   切片参数: {slice_width}x{slice_height}, 重叠: {overlap_width_ratio:.1%}x{overlap_height_ratio:.1%}")
        LOGGER.info(f"   跨类别NMS: {'启用' if cross_class_nms_enabled else '禁用'} (阈值={cross_class_nms_threshold})")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="D1 数据集 SAHI 切片推理脚本 v3 (支持跨标签NMS)")
    ...
    # v3新增: 跨类别NMS参数
    parser.add_argument("--no-cross-class-nms", action="store_true", help="禁用跨类别NMS")
    parser.add_argument("--cross-class-nms-threshold", type=float, default=0.5, help="跨类别NMS的IoU阈值")
    ...
    try:
        # 创建推理器
        LOGGER.info("🚀 初始化 SAHI 推理器 v3 (支持跨标签NMS)...")
        ...
        cross_class_nms_enabled = not args.no_cross_class_nms
        ...
        if source_path.is_file():
            ...
            result = inferencer.predict_image(
                ...
                cross_class_nms_enabled=cross_class_nms_enabled,
                cross_class_nms_threshold=args.cross_class_nms_threshold,
