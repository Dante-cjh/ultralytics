...
################################################################################
# D1 数据集 - 多模型训练脚本
# 依次训练 YOLOv8m, YOLOv8l, YOLOv8x 模型
...
# 数据路径
VAL_DIR="/public/home/baichen/download/dcu_yolo/ultralytics/data/D1_type3/yolo_format/images/val"
...
log_info "🚀 开始 D1 多模型训练流水线"
log_info "时间戳: ${TIMESTAMP}"
log_info "训练参数: epochs=${EPOCHS}, batch=${BATCH}, patience=${PATIENCE}, device=${DEVICE}"

# 定义模型列表
declare -a MODELS=("yolov8m.pt" "yolov8l.pt" "yolov8x.pt")
declare -a MODEL_NAMES=("yolov8m_2560" "yolov8l_2560" "yolov8x_2560")
...
for i in "${!MODELS[@]}"; do
    ...
    PROJECT_NAME="D1_${MODEL_NAME}_${TIMESTAMP}"
    ...
    python3 balloon_inference.py \
        --model "${BEST_MODEL}" \
        --data "/public/home/baichen/download/dcu_yolo/ultralytics/my_D1.yaml" \
        --val \
        --batch "${BATCH}" \
        --imgsz 2560 \
        --confidence ${CONFIDENCE} \
