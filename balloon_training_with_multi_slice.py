...
class BalloonTrainingPipeline:
    ...
    def __init__(
        ...
        project_name: str = "balloon_yolo11n_multi_slice"
    ):
        """
        初始化训练流水线
    ...
    def train_model(
        ...
        # 使用预定义的数据集配置文件
        dataset_yaml = "balloon_multi_slice.yaml"
    ...
    def run_complete_pipeline(
        ...
        Returns:
            bool: 是否成功
        """
        LOGGER.info("🎯 开始 Balloon 完整训练流水线 (多尺度版本)")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="Balloon 数据切片和训练集成脚本 (多尺度版本)")
    
    # 数据参数
    parser.add_argument("--data-root", type=str, 
                       default="/home/cjh/mmdetection/data/balloon/yolo_format",
                       help="原始 Balloon 数据根目录")
    parser.add_argument("--slice-dir", type=str, 
                       default="/home/cjh/mmdetection/data/balloon/yolo_format_multi_slice",
                       help="切片后数据保存目录")
    parser.add_argument("--project-name", type=str, default="balloon_yolo11n_multi_slice", help="训练项目名称")
    
    # 切片参数
    parser.add_argument("--crop-size", type=int, default=640, help="切片窗口大小")
    parser.add_argument("--gap", type=int, default=100, help="窗口重叠大小")
    parser.add_argument("--rates", nargs="+", type=float, default=[0.5, 1.0, 1.5], help="多尺度缩放比例")
