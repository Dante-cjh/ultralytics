...
# Balloon数据集路径
NEW_DATA_DIR="/home/cjh/mmdetection/data/balloon/val"

# 输出根目录
OUTPUT_ROOT="runs/balloon_comprehensive_test_$(date +%Y%m%d)"

# GPU设备
DEVICE=0

# 三个模型路径（使用相同的模型进行测试）
NORMAL_MODEL="runs/detect/balloon_yolo11l_20251203_160322/weights/best.pt"
SLICE_MODEL="runs/detect/balloon_yolo11l_20251203_160322/weights/best.pt"
MULTISCALE_MODEL="runs/detect/balloon_yolo11l_20251203_160322/weights/best.pt"
...
run_normal_inference() {
    ...
    if [ $? -eq 0 ]; then
        log_info "✅ 常规推理完成"
        # balloon_inference.py 将标签保存在 predict/labels 目录
        count_detections "${save_dir}/predict/labels" "常规推理"
    else
        log_error "❌ 常规推理失败"
        return 1
    fi
...
run_sahi_inference() {
    ...
    log_section "🔍 方法2: SAHI切片推理 (支持跨标签NMS)"
    log_info "模型: ${model_path}"
    log_info "数据: ${source_dir}"
    log_info "保存: ${save_dir}"
...
run_multiscale_inference() {
    ...
    log_section "🔍 方法3: 多尺度推理 (支持跨标签NMS)"
    log_info "模型: ${model_path}"
    log_info "数据: ${source_dir}"
    log_info "保存: ${save_dir}"
    log_info "尺度: ${SCALES}"
...
log_section "🚀 Balloon 数据集综合测试"
log_info "测试时间: ${TIMESTAMP}"
log_info "数据集: ${NEW_DATA_DIR}"
log_info "保存路径: ${OUTPUT_ROOT}"

# 验证输入
if [ ! -d "${NEW_DATA_DIR}" ]; then
    log_error "数据集目录不存在: ${NEW_DATA_DIR}"
    exit 1
fi
...
if [ -f "${NORMAL_MODEL}" ]; then
    run_normal_inference "${NORMAL_MODEL}" "${NEW_DATA_DIR}" \
        "${OUTPUT_ROOT}/01_normal_inference_${TIMESTAMP}" || log_error "常规推理失败，继续下一步"
else
...
if [ -f "${SLICE_MODEL}" ]; then
    run_sahi_inference "${SLICE_MODEL}" "${NEW_DATA_DIR}" \
        "${OUTPUT_ROOT}/02_sahi_inference_${TIMESTAMP}" || log_error "SAHI推理失败，继续下一步"
else
...
# 显示各方法的输出路径
if [ -d "${OUTPUT_ROOT}/01_normal_inference_${TIMESTAMP}" ]; then
    log_info "  1️⃣  常规推理:"
    log_info "     图像: ${OUTPUT_ROOT}/01_normal_inference_${TIMESTAMP}/*_visual.jpg"
    log_info "     标签: ${OUTPUT_ROOT}/01_normal_inference_${TIMESTAMP}/labels/*.txt"
    log_info "     统计: ${OUTPUT_ROOT}/01_normal_inference_${TIMESTAMP}/detection_stats.txt"
    log_info ""
fi

if [ -d "${OUTPUT_ROOT}/02_sahi_inference_${TIMESTAMP}" ]; then
    log_info "  2️⃣  SAHI切片推理 (跨标签NMS):"
    log_info "     图像: ${OUTPUT_ROOT}/02_sahi_inference_${TIMESTAMP}/*_visual.jpg"
    log_info "     标签: ${OUTPUT_ROOT}/02_sahi_inference_${TIMESTAMP}/labels/*.txt"
    log_info "     统计: ${OUTPUT_ROOT}/02_sahi_inference_${TIMESTAMP}/detection_stats.txt"
    log_info ""
fi

if [ -d "${OUTPUT_ROOT}/03_multiscale_inference_${TIMESTAMP}" ]; then
    log_info "  3️⃣  多尺度推理 (跨标签NMS):"
    log_info "     图像: ${OUTPUT_ROOT}/03_multiscale_inference_${TIMESTAMP}/*_multiscale.jpg"
    log_info "     标签: ${OUTPUT_ROOT}/03_multiscale_inference_${TIMESTAMP}/labels/*.txt"
    log_info "     统计: ${OUTPUT_ROOT}/03_multiscale_inference_${TIMESTAMP}/detection_stats.txt"
    log_info ""
fi
...
log_info ""
log_info "💡 查看可视化图片："
log_info "  ls ${OUTPUT_ROOT}/*/*.jpg"
log_info ""
log_info "✅ 测试脚本验证完成！"
log_info "   可以将此脚本配置修改为D1数据集路径使用"
log_info ""
